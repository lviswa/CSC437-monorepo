...
function getMongoURI(dbname) {
  ...
  if (MONGO_USER && MONGO_PWD && MONGO_CLUSTER) {
    console.log(
      `Connecting to MongoDB cluster ${MONGO_CLUSTER} as ${MONGO_USER}`
    );
    connection_string = `mongodb+srv://${MONGO_USER}:${MONGO_PWD}@${MONGO_CLUSTER}/${dbname}?retryWrites=true&w=majority`;
  } else {
    console.log("Connecting to local MongoDB at", connection_string);
  }
  return connection_string;
}
function connect(dbname) {
  import_mongoose.default.connect(getMongoURI(dbname)).catch(
